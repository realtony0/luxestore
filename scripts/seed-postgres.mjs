/**
 * Imports products from data/products.json into Postgres (Neon).
 * Usage: DATABASE_URL=postgresql://... node scripts/seed-postgres.mjs
 * Or with .env.local loaded: node -r dotenv/config scripts/seed-postgres.mjs
 */
...
async function main() {
  ...
  console.log(`${inserted} product(s) imported.`);
}
