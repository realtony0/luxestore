...
function toPayload(form: ProductFormState): Omit<Product, "id" | "slug"> {
  ...
  const requiresModeSubcategory = form.universe === "mode" && modeCategory === "Clothes";
  if (requiresModeSubcategory && !subCategory) {
    throw new Error("Choose a subcategory for Clothes.");
  }
...
export default function AdminBoutique() {
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      setProductsError("Network error while deleting product.");
    } finally {
      setDeletingId(null);
    }
  }
  ...
  async function handleDeleteCategory(category: CategoryInfo) {
    ...
    if (category.count > 0) {
      setCategoryError(`Cannot delete "${category.name}" because this category contains products.`);
      return;
    }
  ...
  async function handleRenameCategory(category: CategoryInfo) {
    ...
    if (!nextName) {
      setCategoryError("New category name is required.");
      return;
    }
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setCategoryError(data.error || "Failed to rename category.");
        return;
      }
  ...
  async function handleCreateModeSubcategory() {
    ...
    if (!name) {
      setModeSubcategoryError("Subcategory name is required.");
      return;
    }
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setModeSubcategoryError(data.error || "Failed to create subcategory.");
        return;
      }
  ...
  async function handleDeleteModeSubcategory(subcategory: ModeSubcategoryInfo) {
    ...
    const confirmMessage =
      subcategory.count > 0
        ? `Delete subcategory "${subcategory.name}"? Linked products will move to "Clothes" without a subcategory.`
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setModeSubcategoryError(data.error || "Failed to delete subcategory.");
        return;
      }
  ...
  async function handleRenameModeSubcategory(subcategory: ModeSubcategoryInfo) {
    ...
    if (!nextName) {
      setModeSubcategoryError("New subcategory name is required.");
      return;
    }
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setModeSubcategoryError(data.error || "Failed to rename subcategory.");
        return;
      }
  ...
  const isModeClothingCategory = form.universe === "mode" && normalizeModeCategoryInput(form.category) === "Clothes";
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
      <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="font-heading text-2xl font-semibold text-[var(--foreground)]">Store administration</h1>
          <p className="mt-1 text-sm text-[var(--muted)]">
            Control products and categories: create, edit, delete, and filter.
          </p>
        </div>
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-3">
        ...
        <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-4">
          ...
          <p className="mt-2 text-sm text-[var(--foreground)]">
            Fashion: <span className="font-semibold">{modeCount}</span> · Lifestyle: <span className="font-semibold">{toutCount}</span>
          </p>
        </div>
        <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-4">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--muted)]">Categories</p>
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[var(--border)] bg-[var(--card)] p-3">
        ...
        <p className="text-xs text-[var(--muted)]">
          Active view: <span className="font-semibold text-[var(--foreground)]">{adminView === "products" ? "Products" : "Categories"}</span>
        </p>
      </div>
      ...
      <section className="mb-6 rounded-xl border border-[var(--border)] bg-[var(--card)] p-4">
        <div className="flex flex-wrap items-end gap-3">
          <div className="min-w-56 flex-1">
            <label className="block text-xs font-semibold uppercase tracking-[0.14em] text-[var(--muted)]">New category</label>
            <input
              type="text"
              value={newCategory}
              onChange={(e) => setNewCategory(e.target.value)}
              placeholder="Ex. Clothes, Shoes, Wigs"
          ...
          >
            {categoryLoading ? "Adding..." : "Create category"}
          </button>
        </div>
        ...
        {loading ? (
          <p className="mt-4 text-sm text-[var(--muted)]">Loading categories...</p>
        ) : (
          <div className="mt-4 overflow-x-auto rounded-lg border border-[var(--border)]">
            <table className="w-full border-collapse bg-white">
              <thead>
                <tr className="bg-[var(--background)]">
                  <th className="border-b border-[var(--border)] p-3 text-left text-xs font-semibold uppercase tracking-[0.14em] text-[var(--muted)]">Category</th>
                  ...
                  <th className="border-b border-[var(--border)] p-3 text-left text-xs font-semibold uppercase tracking-[0.14em] text-[var(--muted)]">Rename</th>
              ...
              <tbody>
                {categories.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="p-4 text-sm text-[var(--muted)]">No categories.</td>
                  </tr>
                ) : (
                    ...
                    <tr key={category.name} className="border-b border-[var(--border)] last:border-0">
                      ...
                      <td className="p-3">
                        <div className="flex items-center gap-2">
                          ...
                          >
                            {renamingCategory === category.name ? "…" : "Rename"}
                          </button>
                        </div>
                      </td>
                      <td className="p-3 text-right">
                        ...
                        >
                          {deletingCategory === category.name ? "…" : "Delete"}
                        </button>
                      </td>
                    </tr>
                  ))
        ...
        <div className="mt-8 border-t border-[var(--border)] pt-6">
          <div className="flex flex-wrap items-end gap-3">
            ...
            <button
              ...
              className="rounded-lg bg-[var(--foreground)] px-4 py-2 text-sm font-medium text-white transition hover:opacity-90 disabled:opacity-70"
            >
              {modeSubcategoryLoading ? "Adding..." : "Create subcategory"}
            </button>
          </div>
          ...
          {loading ? (
            <p className="mt-4 text-sm text-[var(--muted)]">Loading subcategories...</p>
          ) : (
            <div className="mt-4 overflow-x-auto rounded-lg border border-[var(--border)]">
              <table className="w-full border-collapse bg-white">
                <thead>
                        ...
                        <td className="p-3">
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              className="rounded border border-[var(--border)] px-2.5 py-1.5 text-xs font-medium text-[var(--foreground)] transition hover:border-[var(--accent)] hover:text-[var(--accent)] disabled:opacity-50"
                            >
                              {renamingModeSubcategory === subcategory.name ? "…" : "Rename"}
                            </button>
                          </div>
                        </td>
                        <td className="p-3 text-right">
                          <button
                            ...
                            className="rounded border border-[var(--border)] px-3 py-1.5 text-xs font-medium text-[var(--accent-deep)] transition hover:border-[var(--accent-deep)] disabled:opacity-50"
                          >
                            {deletingModeSubcategory === subcategory.name ? "…" : "Delete"}
                          </button>
                        </td>
                      </tr>
                    ))
      ...
      {adminView === "products" && (
        <div className="mb-6 grid gap-3 rounded-xl border border-[var(--border)] bg-[var(--card)] p-4 sm:grid-cols-3">
          <div>
            <label className="block text-xs font-semibold uppercase tracking-[0.14em] text-[var(--muted)]">Search</label>
            <input
              type="text"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="Name, slug, color, category..."
              className="mt-1 w-full rounded-lg border border-[var(--border)] px-3 py-2 text-sm text-[var(--foreground)]"
            />
          </div>
          <div>
            <label className="block text-xs font-semibold uppercase tracking-[0.14em] text-[var(--muted)]">Universe</label>
            <select
            ...
            >
              ...
              <option value="tout">Universe</option>
            </select>
          </div>
          <div>
            <label className="block text-xs font-semibold uppercase tracking-[0.14em] text-[var(--muted)]">Sort</label>
            <select
              value={sortBy}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 p-4">
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl border border-[var(--border)] bg-[var(--card)] p-6 shadow-xl">
            ...
            <form onSubmit={handleSubmit} className="mt-4 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--foreground)]">Price (USD) *</label>
                <input
                  type="number"
                  min={0}
                  value={form.price}
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <label className="block text-sm font-medium text-[var(--foreground)]">Universe *</label>
                  <select
                    value={form.universe}
                    onChange={(e) => {
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setForm((f) => ({
                          ...
                          subCategory: e.target.value === "Clothes" ? f.subCategory : "",
                        }))
                      }
                      required
                      className="mt-1 w-full rounded-lg border border-[var(--border)] px-3 py-2 text-[var(--foreground)]"
                ...
                {isModeClothingCategory && (
                  <div>
                    <label className="block text-sm font-medium text-[var(--foreground)]">Clothing subcategory *</label>
                    <select
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--foreground)]">Product images *</label>
                <div className="mt-1 rounded-lg border border-[var(--border)] p-3">
                  ...
                  {imageUploading && (
                    <p className="mt-2 text-xs font-medium text-[var(--muted)]">Uploading images...</p>
                  )}
                  {imageUploadError && (
                ...
                <div>
                  ...
                  <div className="mt-1 flex gap-2">
                    <input
                      ...
                      placeholder="Ex. Black or #111111"
                  ...
                  <p className="mt-1 text-[11px] text-[var(--muted)]">
                    Enter a color then click &quot;Add&quot;. HEX codes are supported (e.g. #111111).
                  </p>
                          ...
                          >
                            ...
                            {colorImages.length === 0 ? (
                              <p className="text-[11px] text-[var(--muted)]">
                                No specific image for this color.
                              </p>
                            ) : (
      ...
      {adminView === "products" && (
        loading ? (
          ...
          <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-12 text-center">
            <p className="text-[var(--muted)]">No products found with these filters.</p>
            <button
              type="button"
              onClick={openCreateForm}
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-[var(--background)]">
                  ...
                  <th className="border-b border-[var(--border)] p-3 text-left text-xs font-semibold uppercase tracking-[0.14em] text-[var(--muted)]">Name</th>
                  <th className="border-b border-[var(--border)] p-3 text-left text-xs font-semibold uppercase tracking-[0.14em] text-[var(--muted)]">Price</th>
                  <th className="border-b border-[var(--border)] p-3 text-left text-xs font-semibold uppercase tracking-[0.14em] text-[var(--muted)]">Category</th>
                  <th className="border-b border-[var(--border)] p-3 text-left text-xs font-semibold uppercase tracking-[0.14em] text-[var(--muted)]">Universe</th>
                  <th className="border-b border-[var(--border)] p-3 text-right text-xs font-semibold uppercase tracking-[0.14em] text-[var(--muted)]">Actions</th>
                </tr>
              </thead>
              <tbody>
