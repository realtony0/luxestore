...
export async function createModeSubcategory(
  ...
  if (isModeTopCategory(name)) {
    throw new Error("This value already exists as a top-level Fashion category.");
  }
...
export async function deleteCategory(
  ...
  if (usageCount > 0) {
    if (!replacement) {
      throw new Error("This category contains products. Choose a replacement category.");
    }
...
export async function deleteModeSubcategory(
  ...
  if (!name) throw new Error("Subcategory name is required.");
  if (replacement && replacement === name) {
    throw new Error("Replacement subcategory must be different.");
  }
  ...
  if (usageCount > 0) {
    const fallbackCategory = replacement || "Clothes";
...
export async function renameModeSubcategory(
  ...
  if (!name) throw new Error("Subcategory name is required.");
  if (!nextName) throw new Error("New subcategory name is required.");
  if (isModeTopCategory(nextName)) {
    throw new Error("This name already matches a top-level Fashion category.");
  }
